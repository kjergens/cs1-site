...
!!! attention
    ...
    1. **Vocabulary.** Using these terms, fill in the blanks below with the corresponding definition:
    ...
    2. Fill in the missing parts of this for loop, which is intended to run 10 times:
    ```java
    for (__________ = 0; __________ < 10; __________) {
        // code (not shown)
    }
    ```
    3. The `__________` section runs only once at the beginning of the loop.
    4. If the `__________` is false at the start, the loop body won't execute even once.
    5. The `__________` happens at the end of each iteration to modify the loop variable.
    6. For loops are ideal when you know the exact `__________` of times you want to repeat code.
    7. Omitting the curly braces `{}` in a for loop causes only `__________` statement to repeat, instead of a whole block.
    ...
    ```
    9. What happens if you forget the update statement in a for loop?
    10. Rewrite this while loop as an equivalent for loop. Keep the behavior identical.
    ```java
    int count = 0;
    ...
    ```
    11. Rewrite this while loop as an equivalent for loop. Keep the behavior identical.
    ```java
    int num = 10;
