...
!!! attention
    ...
    Assume `int a = 10; int b = 3; double x = 10.0;` are declared. Write the result of each expression below. If the result is a decimal, include the `.0`.

    1. `a + b`
    2. `a - b`
    3. `a * b`
    4. `a / b`
    5. `x / b` (compare to problem 4 — why are the answers different?)
    6. `a % b`
    7. `a % 2`
    8. `b % a`
